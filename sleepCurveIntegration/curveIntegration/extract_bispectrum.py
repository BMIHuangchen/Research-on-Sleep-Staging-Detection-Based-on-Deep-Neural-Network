...
def get_bi_spectrum(file_folder, class_list, data_num=2000):
    ...
    for class_nam in class_list:
        ...
        count = 0
        for name in tqdm(all_files[:data_num], desc='Processing:'):
            ...
            bi_spectrum = bi_spectrum.reshape((256, 256))
            bi_spectrum = 255 * (bi_spectrum - np.min(bi_spectrum)) / (np.max(bi_spectrum) - np.min(bi_spectrum))
            dataset.append(np.array([bi_spectrum]).astype('float32'))  # concat the dataset
            count+=1
        count_list.append(count)
        print(class_nam+' Samples num:', count)
    return dataset, count_list
