...
def main():
    ...
    for i in range(len(psg_fnames)):
        raw_train = mne.io.read_raw_edf(psg_fnames[i], stim_channel='marker',
                                        misc=['rectal'])
        ...
        # 想要分析的epoch
        epochIndex = 0
        for j in range(0, bandIndex):
            # 计算FIR滤波后的数据并绘图（注意这里要使用copy方法，否则会改变原始数据）
            firFilter = epochs.copy().filter(bandFreqs[j]['fmin'], bandFreqs[j]['fmax'])
            # 对FIR滤波后的数据进行FFT变换
        ...
        GammaY = merge(Gamma, GammaNormalized, 0.1, AlphaY)
